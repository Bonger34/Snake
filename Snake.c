...
void initUI()	// 初始化界面 
{
	...
	for(i = 0; i < snake.len; i++)	// 画蛇 
	{
		...
		if(i == 0)
		{
			SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 0x0A);			// 设置文本颜色为黑底绿字 
			putchar('O');	// 画蛇头 
		}
		else
		{
			if(i % 2 != 0) 
				SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 0x0E);	// 设置文本颜色为黑底黄字 
			else
				SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 0x0A);	// 设置文本颜色为黑底绿字 
			putchar('o');	// 画蛇尾 
		}	
	...
	SetConsoleCursorPosition(GetStdHandle(STD_OUTPUT_HANDLE), coord);		// 设置光标位置 
	SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 0x0C);			// 设置文本颜色为黑底红字 
	putchar('@');			// 画食物 
	SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 0x0F);			// 将文本颜色改回黑底白字 
}
...
void playGame()	// 游戏进行
{
		  ...
		  snake.body[0].Y >= 0 && snake.body[0].Y < HEIGHT)	// 判断蛇是否撞墙 
	{
		...
		{
    		speed = sleepTime / 10;  	// 加速：速度变为原来一半
    		if(speed < 30)
				speed = 30; 			// 限制最小速度
		}
...
int main()
{
	...
	printf(">>>>> Game Over! <<<<<\n\n");
	printf("Your score is %d!", score);	// 打印成绩 
	return 0;
}
